...
PII_REGEX_PATTERNS = [
    ...
    (r'\b\d{1,5}\s+\w+(?:\s+\w+)*\s+(?:St|Street|Ave|Avenue|Rd|Road|Blvd|Boulevard|Ln|Lane|Dr|Drive|Ct|Court)\b\.?', 'address'),
    (r'\b[A-Z]{2}\s+\d{5}(?:-\d{4})?\b', 'location_zip'),
    (r'\b[A-Z][a-zA-Z0-9]+\s+(?:Inc|Corp|LLC|Ltd)\b\.?', 'organization'),
]
